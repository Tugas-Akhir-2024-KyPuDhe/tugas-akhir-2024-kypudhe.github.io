...
export const Table: React.FC = () => {
  ...
  const getData = async () => {
    setLoading(true);
    try {
      const response = await facilityService.all();
  ...
  const handleDelete = async (id: number) => {
    const result = await showConfirmationDialog({
      title: "Ingin menghapus Fasilitas ini?",
      icon: "warning",
    ...
    if (result.isConfirmed) {
      ...
      try {
        const response = await facilityService.destroy(id);
        ...
        Swal.fire("Gagal", "Terjadi kesalahan saat menghapus fasilitas", "error");
      } finally {
