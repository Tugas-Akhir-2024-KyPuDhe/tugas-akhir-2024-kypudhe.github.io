import React from "react";
import DataTable from "react-data-table-component";
import { StyleSheetManager } from "styled-components";
import { FaEye, FaTrash } from "react-icons/fa6";
import { useNavigate } from "react-router-dom";
import { IProblem } from "../../../../interface/problem.interface";
import { convertStatusProblem } from "../../../../utils/myFunctions";

type TableProblemReportProps = {
  loading: boolean;
  data: IProblem[];
  keySearch: string;
  handleChangekeySearch: (keySearch: string) => void;
  handleDetelete: (id: number) => void;
};

export const TableProblemReport: React.FC<TableProblemReportProps> = ({
  loading,
  data,
  keySearch,
  handleChangekeySearch,
  handleDetelete,
}) => {
  const navigate = useNavigate();

  const columns = [
    {
      name: "No",
      cell: (_row: IProblem, index: number) => (
        <div className="w-100 text-center">{index + 1}</div>
      ),
      width: "50px",
    },
    {
      name: "Masalah Halaman",
      selector: (row: IProblem) => row.pageProblem,
      sortable: true,
      cell: (row: IProblem) => (
        <div className="w-100 text-center">{row.pageProblem}</div>
      ),
      width: "165px",
    },
    {
      name: "Deskripsi Masalah",
      selector: (row: IProblem) => row.problemDescription,
      sortable: true,
      cell: (row: IProblem) => row.problemDescription,
    },
    {
      name: "No WA",
      selector: (row: IProblem) => row.telp,
      sortable: true,
      cell: (row: IProblem) => row.telp,
      width: "140px",
    },
    {
      name: "Foto",
      selector: (row: IProblem) => row.idName,
      sortable: true,
      cell: (row: IProblem) => (
        <div className="w-100 text-center p-2">
          {row.media ? (
            <img src={row.media.url} className="img-fluid" width="700" alt="" />
          ) : '-'}
        </div>
      ),
      width: "120px",
    },
    {
      name: "Status",
      selector: (row: IProblem) => row.status,
      sortable: true,
      cell: (row: IProblem) => convertStatusProblem(row.status),
      width: "100px",
    },
    {
      name: "Action",
      selector: (row: IProblem) => row.idName,
      cell: (row: IProblem) => (
        <div className="w-100 text-center">
          <button
            className="btn btn-info btn-sm text me-2 text-light"
            onClick={() => navigate(`detail/${row.id}`)}
            disabled={loading}
          >
            <FaEye />
          </button>
          <button
            className="btn btn-danger btn-sm"
            onClick={() => handleDetelete(row.id)}
            disabled={loading}
          >
            <FaTrash />
          </button>
        </div>
      ),
      width: "150px",
    },
  ];

  const filteredST = data.filter((st) =>
    st.idName.toLowerCase().includes(keySearch.toLowerCase())
  );

  return (
    <div
      className="shadow p-4 m-1 m-lg-4 m-md-4 my-4 rounded"
      style={{ backgroundColor: "#fff", position: "relative" }}
    >
      {loading && (
        <div
          style={{
            position: "absolute",
            top: 0,
            left: 0,
            right: 0,
            bottom: 0,
            backgroundColor: "rgba(255, 255, 255, 0.7)",
            zIndex: 20,
            display: "flex",
            justifyContent: "center",
            alignItems: "center",
          }}
        >
          <div className="spinner-border text-primary" role="status">
            <span className="visually-hidden">Loading...</span>
          </div>
        </div>
      )}

      <div className="row d-flex justify-content-end">
        <div className="col-12 col-lg-4 col-md-3">
          <input
            type="text"
            className="form-control py-2 border-dark"
            placeholder="Search.."
            value={keySearch}
            onChange={(e) => handleChangekeySearch(e.target.value)}
            style={{ fontSize: "1.1em" }}
          />
        </div>
      </div>

      <StyleSheetManager>
        <DataTable
          columns={columns}
          data={filteredST}
          pagination
          highlightOnHover
          className="mt-3"
          customStyles={{
            rows: {
              style: {
                "&:hover": {
                  backgroundColor: "#f5f5f5",
                  color: "#007bff",
                },
              },
            },
            headCells: {
              style: {
                backgroundColor: "var(--blue-color)",
                color: "#ffffff",
                fontWeight: "bold",
                textAlign: "center",
                border: "0.1px solid #ddd",
              },
            },
          }}
        />
      </StyleSheetManager>
    </div>
  );
};
